...
class EditBetViewController: BaseViewController {

    var viewModel: ViewModel?

    private let grabber = UIImageView(image: UIImage(named: "grabber"))
    private let pageTitle = UILabel(text: "Edit bet",
                                    textColor: .white,
    ...
    override func viewWillAppear(_ animated: Bool) {
        ...
        guard let model = self.viewModel?.bet else { return }
        if model.isWin {
        ...
        self.oddsField.text = model.odds
        self.stakeField.text = model.stake
    }
...
extension EditBetViewController: IViewModelableController {
    typealias ViewModel = IEditBetViewModel
}
...
//MARK: Button Actions
extension EditBetViewController {
    ...
    @objc func deleteTapped() {
        guard let id = self.viewModel?.bet.id else { return }
        self.viewModel?.deleteBet(by: id)
    ...
    @objc func doneTapped() {
        ...
        self.viewModel?.editBet(model: .init(id: id,
                                             teamId: team.id!,
                                             teamName: team.name,
...
extension EditBetViewController: UICollectionViewDataSource, UICollectionViewDelegate, UICollectionViewDelegateFlowLayout {
...
struct EditBetViewControllerProvider: PreviewProvider {
    ...
    struct ContainerView: UIViewControllerRepresentable {
        let editBetViewController = EditBetViewController()

        func makeUIViewController(context: UIViewControllerRepresentableContext<EditBetViewControllerProvider.ContainerView>) -> EditBetViewController {
            return editBetViewController
        }

        func updateUIViewController(_ uiViewController: EditBetViewControllerProvider.ContainerView.UIViewControllerType, context: UIViewControllerRepresentableContext<EditBetViewControllerProvider.ContainerView>) {
        }
    }
}
